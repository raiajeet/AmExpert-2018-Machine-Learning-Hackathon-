rm(list=ls())          
library(data.table)
data=fread("C:\\Users\\AJIT\\Documents\\amexpert\\train.csv",stringsAsFactors = T)
data=subset(data,select = -c(session_id,DateTime,user_id,campaign_id,webpage_id))
sum(is.na(data))
library(caret)
...
id=sample(2,nrow(train_transformed),prob=c(.8,.2),replace=TRUE)
newtrain=train_transformed[id==1,]
newtest=train_transformed[id==2,]
#################################################################################################
y.dep <- 19
x.indep <- c(1:18)
library(h2o)
...
gbm.model <- h2o.gbm(y=y.dep, x=x.indep, training_frame = train.h2o,
                     ntrees = 1000, max_depth = 4, learn_rate = 0.01, seed = 1122) #0.5793478
...
print(h2o.auc(h2o.performance(gbm, newdata = test.h2o)))     # 0.5824282
